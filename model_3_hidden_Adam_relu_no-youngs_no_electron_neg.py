# -*- coding: utf-8 -*-
"""
Created on Wed Feb 23 11:18:21 2022
...
class HEM_train(Dataset): #high entropy metal
    def __init__(self):
        ...
        sc = StandardScaler()
        self.x_data = torch.from_numpy(sc.fit_transform(xy[:, [0,1,2,3,4]])) # size [n_samples, n_features]
...
class HEM_val(Dataset): #high entropy metal
    def __init__(self):
        ...
        self.val_x_data = torch.from_numpy(sc.fit_transform(xy_val[:,[0,1,2,3,4]]))
        # self.y_data = torch.from_numpy(xy[:, [13]]) # size [n_samples, 1]
...
def main():
    model=MLP(5,64,128,64,1).to(Device)
